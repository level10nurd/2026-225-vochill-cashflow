...
└────────┼─────────────┼─────────────┼─────────────┼─────────┘
         ...
         │         OUTPUT GENERATORS                │
         │                                          │
         │  • Hex app (dashboard, metrics, scenarios)│
...
### 6. **Output Generation**
- Hex app (dashboard, cash position, runway, 13-week view, scenario comparison)
...
**Next step:** Build the Cash Flow Hex app (see [docs/hex_app/](../hex_app/) for strategy, cell-by-cell guide, and implementation checklist).
...
### Phase 2: Data Access ✅
- [x] BigQuery connector module ([src/data/bigquery_connector.py](../../src/data/bigquery_connector.py))
- [x] Cash flow SQL queries ([src/queries/](../../src/queries/))
- [x] Data aggregation (ETL and [scripts/build_forecast.py](../../scripts/build_forecast.py) populate `cash_transactions`; see [ETL_COMPLETE.md](ETL_COMPLETE.md))

### Phase 3: Forecasting (external)
Forecasting is **out of scope for this repo**. Demand/revenue forecasting lives in a separate Hex project (vochill-forecasting repo). This repo will connect to that project later (e.g. cash flow Hex app consuming revenue inputs when ready). The 13-week cash forecast is generated by [scripts/build_forecast.py](../../scripts/build_forecast.py) and written to `cash_transactions` with `is_forecast = TRUE`.

### Phase 4: Reporting
- [ ] Hex app (dashboard): cash position, runway, 13-week cash flow, scenario comparison
- [ ] Dashboard metrics and variance analysis in Hex
- [ ] Scenario comparison view in Hex
- See [docs/hex_app/](../hex_app/) for implementation plan (patterned after the vochill-forecasting Hex app).

### Phase 5: Deployment
- [ ] Hex project integration and publish
- [ ] Automated refresh schedule (if desired)
- [ ] Documentation
